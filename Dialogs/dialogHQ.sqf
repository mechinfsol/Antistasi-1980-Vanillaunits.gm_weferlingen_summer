...
if (str (_display) != "no display") then
{
	...
	_ChildControl  ctrlSetTooltip format ["Current level: %2. Next Level Training Cost: %1 DM",1000 + (1.5*((skillFIA) *750)),skillFIA];
/*
	...
	if (isNil "_veh") then
		...
		if (_coste == 0) then
			...
			_texto = format ["%2 Price: %1 DM",_coste,_texto];
			};
		};
