...
if (worldName == "Tanoa") then
    ...
    if (worldName == "Altis") then
        ...
        if (worldName == "gm_weferlingen_summer") then
			...
			spawnPoints = ["spawnPoint","spawnPoint_1","spawnPoint_2","spawnPoint_3","spawnPoint_4","spawnPoint_5"];
			recursos = ["resource","resource_1","resource_2","resource_3","resource_4","resource_5","resource_6","resource_7","resource_8","resource_9"];//economic resources
			fabricas = ["factory","factory_1","factory_2","factory_3","factory_4","factory_5","factory_6","factory_7"];//factories
			puestos = ["puesto","puesto_1","puesto_2","puesto_3","puesto_4","puesto_5","puesto_6","puesto_7","puesto_8","puesto_9","puesto_10","puesto_11","puesto_12","puesto_13","puesto_14","puesto_15","puesto_16","Puesto_17"];//any small zone with mil buildings
			puertos = ["port","port_1","port_2","port_3","port_4"];//seaports, adding a lot will affect economics, 5 is ok
			controles = ["control","control_1","control_2","control_3","control_4","control_5","control_6","control_7","control_8","control_9","control_10","control_11","control_12","control_13","control_14","control_15","control_16","control_17","control_18","control_19","control_20","control_21","control_22","control_23","control_24","control_25","control_26","control_27","control_28","control_29","control_30","control_31","control_32","control_33","control_34","control_35","control_36","control_37","control_38","control_39","control_40","control_41","control_42","control_43","control_44","control_45","control_46","control_47","control_48","control_49","control_50","control_51","control_52","control_53","control_54","control_55","control_56","control_57","control_58","control_59","control_60","control_61","control_62"];//use this for points where you want a roadblock (logic/strategic points, such as crossroads, airport or bases entrances etc..) game will add some more automatically
			seaMarkers = [];
			seaSpawn = [];
			seaAttackSpawn = [];
			{
...
if ((_nombre != "") and (_nombre != "Lakatoro01") and (_nombre != "Galili01") and (_nombre != "Sosovu01") and (_nombre != "Ipota01") and (_nombre != "hill12") and (_nombre != "V_broad22")) then//sagonisi is blacklisted in Altis for some reason. If your island has a city in a small island you should blacklist it (road patrols will try to reach it)
    ...
    _roads = [];
    _numCiv = 0;
    if ((worldName != "Tanoa") and (worldName != "Altis") and (worldName != "chernarus")) then//If Tanoa, data is picked from a DB in initVar.sqf, if not, is built on the fly.
        {
...
if (worldName == "Tanoa") then
    ...
    if (worldName == "Altis") then
        ...
        if (worldName == "gm_weferlingen_summer") then
            {
            _posAntenas = [[16357.3,9742.77,0],[6148.66,4416.24,0],[12634.9,14626.1,0],[18683.5,4529.11,0],[15880.1,2266.04,0],[14279.9,15045.6,0],[11029.7,2318.88,0]];
            _blackListPos = [7];
            antenas = [];
            }
        else
            {
            antenas = nearestObjects [[worldSize /2,worldSize/2,0],["Land_TTowerBig_1_F","Land_TTowerBig_2_F","Land_Communication_F","Land_Vysilac_FM","Land_A_TVTower_base","Land_Telek1","land_gm_radiotower_01"], worldSize];
